...
public class StudentPreference implements Comparable<StudentPreference>{
	...
	private int colour;  //Just a number to represent the colour that the student has given to this course. Read only.
	...
	public StudentPreference(Course inp_courseObj,int inp_colour, int inp_preferenceNo){
	...
	public static StudentPreference getStudentPreferenceBycourseNumber(ArrayList<StudentPreference> preferenceList,String inpcourseNumber){
		for (int i=0;i<preferenceList.size();i++){
			if (preferenceList.get(i).courseObj.getcourseNumber() == inpcourseNumber){
