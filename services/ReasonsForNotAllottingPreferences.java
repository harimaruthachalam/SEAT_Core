...
public class ReasonsForNotAllottingPreferences {
	...
	private static String detailedReasonForNotAllottingInvalidPreference(StudentPreference invalidElective, Student s, ArrayList<Student> originalStudentList) {
		//Check if the preference clashes with a core course
		for (Course core : s.coreCourses){
			if (core.slotClashWithCourse(invalidElective.getCourseObj())){
				return "Slot Clash with core course " + core.getcourseNumberToPrint();
			}
		}
		...
		if (invalidElective.getCourseObj().getCredits() > s.getMaxElectiveCreditsInSem()){
			String creditsViolationReason = "For this student max credits in semester is " + s.maxCreditsInSem + " and Core courses registered are ";
			for (Course core : s.coreCourses){
				creditsViolationReason = creditsViolationReason + core.getcourseNumberToPrint() + "(" + core.getCredits() + " credits)+";
			}
			creditsViolationReason = creditsViolationReason + " and " + invalidElective.getCourseObj().getcourseNumberToPrint() + " takes " + invalidElective.getCourseObj().getCredits() + " credits which is greater than the " + s.getMaxElectiveCreditsInSem() + " remaining elective credits.";
	...
	private static String reasonForNotAllottingValidPreference(StudentPreference validPreference, Student s, ArrayList<Student> originalStudentList, boolean detailed) {
		 ...
		 * 0. Already allotted to inside department version of the course
		 * 1. Slot clash with already allotted course, which was ranked higher by student (slot clash with core course makes the course invalid)
		 * 2. Course with same colour code already allotted, which was ranked higher by student
		 ...
		 * 4. Course is full
		 * 5. Slot clash with already allotted course, which was ranked lower by student (will not occour for IterativeHR, firstPreferenceAllotment. Can occour for slotBasedHR)
		...
		//0. Check if already allotted in the inside department version of the course
		reason = checkIfAlreadyAllottedToInsideDeptVersionOfCourse(validPreference,s);
		if (reason!=null){
			return reason;
		}
				
		//1. Check for Slot clash with already allotted course, which was ranked higher by student (slot clash with core course makes the course invalid)
		reason = checkSlotClashWithHigherRankingCourse(validPreference,s);
		if (reason!=null){
			return reason;
		}
		
		//2. Check for Course with same colour code already allotted, which was ranked higher by student
		reason = checkColourClashWithHigherRankingCourse(validPreference,s);
		if (reason!=null){
			return reason;
		}
		...
		//5. Check for Slot clash with already allotted course, which was ranked lower by student (slot clash with core course makes the course invalid)
		reason = checkSlotClashWithLowerRankingCourse(validPreference,s);
		if (reason!=null){
			return reason;
		}
				
		//6. Check for Course with same colour code already allotted, which was ranked lower by student
		reason = checkColourClashWithLowerRankingCourse(validPreference,s);
		if (reason!=null){
			return reason;
		}
	...
	private static String checkIfAlreadyAllottedToInsideDeptVersionOfCourse(StudentPreference validPreference, Student s) {
		for (StudentPreference sp : s.orderedListOfcoursesAllotted){
			if (sp.getCourseObj().getCorrespondingOutsideDepartmentCourse()==validPreference.getCourseObj()){
				return "Already allotted to inside department version of the course";
			}
		}
	...
	private static String checkIfCreditsFinishedWithAllCourses(StudentPreference validPreference, Student s) {
		...
		if (validPreference.getCourseObj().getCredits() > remainingCredits){
			...
			String creditsViolationReason = "For this student max credits in semester is " + s.maxCreditsInSem + " and Core courses registered are ";
			for (Course core : s.coreCourses){
				creditsViolationReason += core.getcourseNumberToPrint() + "(" + core.getCredits() + " credits)+";
			}
			
			//List the Elective credits for electives at a higher preference
			creditsViolationReason += " and the following electives got allotted : ";
			for (StudentPreference allottedElective : s.orderedListOfcoursesAllotted){
				creditsViolationReason += allottedElective.getCourseObj().getcourseNumberToPrint() + "(" + allottedElective.getCourseObj().getCredits() + " credits)+";
			}
			
			//Conclude the reason with the student's credits
			creditsViolationReason += " and " + validPreference.getCourseObj().getcourseNumberToPrint() + " takes " + validPreference.getCourseObj().getCredits() + " credits which is greater than the remaining " + remainingCredits + " elective credits.";
			return creditsViolationReason;
		}
	...
	private static String checkSlotClashWithLowerRankingCourse(StudentPreference validPreference, Student s) {
		for (StudentPreference allottedElective : s.orderedListOfcoursesAllotted){
			if (allottedElective.getCourseObj().slotClashWithCourse(validPreference.getCourseObj()) && allottedElective.getPreferenceNo()>validPreference.getPreferenceNo()){
	...
	private static String checkIfCreditsFinishedWithHigherRankingCoursesInDetail(StudentPreference validPreference, Student s) {
		...
		if (validPreference.getCourseObj().getCredits() > remainingCredits){
			...
			//List the Elective credits for electives at a higher preference
			creditsViolationReason += " and the following electives which have a higher preference got allotted : ";
			for (StudentPreference allottedElective : s.orderedListOfcoursesAllotted){
				if (allottedElective.getPreferenceNo()<validPreference.getPreferenceNo()){
					creditsViolationReason += allottedElective.getCourseObj().getcourseNumberToPrint() + "(" + allottedElective.getCourseObj().getCredits() + " credits)+";
				}
	...
	private static String checkSlotClashWithHigherRankingCourse(StudentPreference validPreference, Student s) {
		for (StudentPreference allottedElective : s.orderedListOfcoursesAllotted){
			if (allottedElective.getCourseObj().slotClashWithCourse(validPreference.getCourseObj()) && allottedElective.getPreferenceNo()<validPreference.getPreferenceNo()){
				return "Slot Clash with already allotted elective course " + allottedElective.getCourseObj().getcourseNumberToPrint();
			}
